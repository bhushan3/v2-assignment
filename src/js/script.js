		...
		$('.swf').each(function () {
			...
			_this.find('.swf__filter-item').on('click', function (e) {
				...
				if (filter == 'all') {
					...
					_this.find('[rel="swf-slide"]').not(`[data-filter='${filter}']`).addClass('is-hidden').removeClass('swiper-slide').hide()
					_this.find(`[rel="swf-slide"][data-filter='${filter}']`).removeClass('is-hidden').addClass('swiper-slide').show()
					swiper.update()
				}
			})
		})
